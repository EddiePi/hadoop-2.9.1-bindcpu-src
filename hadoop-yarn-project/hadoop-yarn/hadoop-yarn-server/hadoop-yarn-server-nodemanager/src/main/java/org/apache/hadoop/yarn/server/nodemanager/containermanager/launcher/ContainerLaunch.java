...
public class ContainerLaunch implements Callable<Integer> {
  ...
  private static final class UnixShellScriptBuilder extends ShellScriptBuilder {
    ...
    public void command(List<String> command) {
      int totalCore = conf.getInt(YarnConfiguration.TOTAL_PHYSICAL_CORE, 8);
      String availabeCoreSet = conf.get(YarnConfiguration.AVAILABLE_CORE_SET, "0-" + String.valueOf(totalCore - 1));
      line("exec taskset -c ", availabeCoreSet, " /bin/bash -c \"", StringUtils.join(" ", command), "\"");
    }
